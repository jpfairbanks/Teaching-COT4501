...
#    u̇ = f(u,p,t)
# where u is the state vector, p is the parameter vector, and t is the current time.
# we call f a time varying parameterized vector field.

# For example, the classic epidemiological model developed by Kermack and McKendrick, also known as the SIR model is time varying parameterized vector field.
# https://en.wikipedia.org/wiki/Kermack%E2%80%93McKendrick_theory
# is a time varying vector field. For further reading see https://www.maa.org/book/export/html/115609.
f(u, p, t) = begin
  ϕ₁ = p[1]*u[1]*u[2]/sum(u)
  ϕ₂ = p[2]*u[2]
  return (-ϕ₁, ϕ₁-ϕ₂, ϕ₂)
...
derivs
plot(tsteps, derivs)
# plot(tsteps, log.(abs.(derivs)).*sign.(derivs))
# plot(tsteps[250:end], derivs[250:end, :])
...
newtoniteration(fsirs₁, Jsirs, soln[:, 250], 3)
